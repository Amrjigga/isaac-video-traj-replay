...
# Finger curl amplification.
# This boosts the existing retargeted finger motion around the default/open hand pose.
parser.add_argument("--finger_curl_boost", type=float, default=1.0)
parser.add_argument("--simple_finger_boost", action="store_true", help="Simple multiplier only: default + (current-default)*boost")
parser.add_argument("--finger_extra_close", type=float, default=0.0)
parser.add_argument("--finger_boost_deadzone", type=float, default=0.015)
parser.add_argument("--finger_boost_start", type=float, default=0.08)
parser.add_argument("--finger_boost_full", type=float, default=0.35)
parser.add_argument("--finger_debug_every", type=int, default=120)
parser.add_argument("--finger_video_gate", action="store_true")
parser.add_argument("--finger_video_curl_start", type=float, default=0.72)
parser.add_argument("--finger_video_curl_full", type=float, default=0.55)
parser.add_argument("--finger_open_return", type=float, default=0.35)
parser.add_argument("--finger_close_hold_frames", type=int, default=12)
parser.add_argument("--finger_open_gate", type=float, default=0.02)
...
def _find_existing_joints(candidate_names):
    found_ids = []
    found_names = []

    for name in candidate_names:
        try:
            ids, names = robot.find_joints([name])
            if len(ids) > 0:
                found_ids.append(ids[0])
                found_names.append(names[0])
        except Exception:
            pass
...
def _init_finger_boost_joints():
    if _finger_boost_state["right_ids"] is not None:
        return
    ...
    r_ids, r_names = _find_existing_joints(right_candidates)
    l_ids, l_names = _find_existing_joints(left_candidates)

    _finger_boost_state["right_ids"] = r_ids
    _finger_boost_state["right_names"] = r_names
    _finger_boost_state["left_ids"] = l_ids
    _finger_boost_state["left_names"] = l_names

    print("[FINGER BOOST] right ids:", r_ids)
    print("[FINGER BOOST] right names:", r_names)
    print("[FINGER BOOST] left ids:", l_ids)
    print("[FINGER BOOST] left names:", l_names)


def _clamp_to_joint_limits(q, joint_ids):
    try:
        limits = robot.data.soft_joint_pos_limits[0, joint_ids]
        lo = limits[:, 0].unsqueeze(0)
        hi = limits[:, 1].unsqueeze(0)
...
def _video_hand_curl_gate(side):
    """
    Returns 0..1 from video landmarks.
    0 = hand looks open
    1 = hand clearly curled/closed

    Uses fingertip distance from wrist relative to MCP distance from wrist.
    Smaller tip/wrist ratio means more curled.
    """
    if not args_cli.finger_video_gate:
        return 1.0
    ...
    fingers = [
        ("INDEX_FINGER_MCP", "INDEX_FINGER_TIP"),
    ...
    curls = []
    for mcp_name, tip_name in fingers:
        mcp = lms.get(mcp_name)
        tip = lms.get(tip_name)
        if not valid(mcp) or not valid(tip):
            continue

        mcp_t = torch.tensor(mcp, dtype=torch.float32)
        tip_t = torch.tensor(tip, dtype=torch.float32)

        mcp_d = torch.linalg.norm(mcp_t - wrist_t)
        tip_d = torch.linalg.norm(tip_t - wrist_t)

        if float(mcp_d) < 1e-5:
            continue

        ratio = float(tip_d / mcp_d)

        # Open fingers usually have larger ratio.
        # Curled fingers have smaller ratio.
        gate = (args_cli.finger_video_curl_start - ratio) / max(
            1e-6,
            args_cli.finger_video_curl_start - args_cli.finger_video_curl_full,
        )
...
def _get_finger_command_source(joint_ids, fallback_q_cur):
    """
    Prefer the current commanded joint target from the original retargeter.
    This prevents simple boost from reinforcing stale closed physical positions.
    """
    candidates = [
        ("robot.data.joint_pos_target", lambda: robot.data.joint_pos_target[:, joint_ids]),
        ("robot._joint_pos_target", lambda: robot._joint_pos_target[:, joint_ids]),
        ("robot._data.joint_pos_target", lambda: robot._data.joint_pos_target[:, joint_ids]),
    ]
    ...
    return fallback_q_cur



def _apply_finger_curl_boost_for_side(side):
    """
    Boosts existing finger motion around default/open pose.

    If the old retargeting produced:
      q_current = default + small curl_delta

    This sends:
      q_target = default + curl_delta * boost + optional extra_close
    """
    if args_cli.finger_curl_boost == 1.0 and args_cli.finger_extra_close == 0.0:
        return

    _init_finger_boost_joints()

    if side == "right":
        joint_ids = _finger_boost_state["right_ids"]
    else:
        joint_ids = _finger_boost_state["left_ids"]
    ...
    delta = q_cur - q_default

    # Simple boost mode:
    # Leaves the original finger behavior intact, but slightly amplifies
    # whatever curl the existing retargeter already produced.
    # No video gates, no hold logic, no open/close memory.
    if args_cli.simple_finger_boost:
        # Use the original retargeter's current target, not stale physical q_cur.
        q_source = _get_finger_command_source(joint_ids, q_cur)
        source_delta = q_source - q_default

        boosted = q_default + source_delta * args_cli.finger_curl_boost
        boosted = _clamp_to_joint_limits(boosted, joint_ids)
        robot.set_joint_position_target(boosted, joint_ids=joint_ids)

        if step % args_cli.finger_debug_every == 0 and side == "right":
            print(
                "[SIMPLE FINGER BOOST]",
                "side=", side,
                "boost=", args_cli.finger_curl_boost,
                "source_delta_mean=", round(float(torch.mean(torch.abs(source_delta))), 4),
            )
        return

    # Adaptive boost:
    # - open/small curl: no boost
    # - stronger curl: gradually blend toward full boost
    abs_delta = torch.abs(delta)
    direction = torch.sign(delta)

    denom = max(1e-6, args_cli.finger_boost_full - args_cli.finger_boost_start)
    boost_gate = (abs_delta - args_cli.finger_boost_start) / denom
    boost_gate = torch.clamp(boost_gate, 0.0, 1.0)

    # Video gate:
    #   0 = video hand looks open
    #   1 = video hand looks clearly curled/closed
    video_gate = _video_hand_curl_gate(side)

    # Hysteresis:
    # Once video curl becomes meaningful, keep the close behavior alive for a few frames.
    hold_key = "right_hold" if side == "right" else "left_hold"

    if args_cli.finger_video_gate and video_gate > args_cli.finger_open_gate:
        _finger_boost_state[hold_key] = args_cli.finger_close_hold_frames
    else:
        _finger_boost_state[hold_key] = max(0, _finger_boost_state[hold_key] - 1)

    close_active = (not args_cli.finger_video_gate) or video_gate > args_cli.finger_open_gate or _finger_boost_state[hold_key] > 0

    # If video hand is clearly open for long enough, actively return fingers toward default/open pose.
    if args_cli.finger_video_gate and not close_active:
        boosted = q_cur + (q_default - q_cur) * args_cli.finger_open_return
    else:
        # During hold frames, keep at least a small gate so the grip does not instantly release.
        gate_for_boost = max(video_gate, 0.25 if _finger_boost_state[hold_key] > 0 else 0.0)

        effective_boost = 1.0 + boost_gate * gate_for_boost * (args_cli.finger_curl_boost - 1.0)
        boosted = q_default + delta * effective_boost

        moving = abs_delta > args_cli.finger_boost_start
        if args_cli.finger_extra_close != 0.0:
            boosted = boosted + moving.float() * direction * args_cli.finger_extra_close * boost_gate * gate_for_boost
    ...
    if step % args_cli.finger_debug_every == 0 and side == "right":
        print(
            ...
            "side=", side,
            "boost=", args_cli.finger_curl_boost,
            "extra=", args_cli.finger_extra_close, "start=", args_cli.finger_boost_start, "full=", args_cli.finger_boost_full,
            "video_gate=", round(float(video_gate), 3), "hold=", _finger_boost_state["right_hold" if side == "right" else "left_hold"], "q_cur=", [[round(float(v), 3) for v in row] for row in q_cur.detach().cpu().tolist()],
            "q_target=", [[round(float(v), 3) for v in row] for row in boosted.detach().cpu().tolist()],
        )



def _reset_fingers_to_default_on_video_loop():
    global _video_prev_frame_idx

    cur_idx = _video_frame_index()

    if _video_prev_frame_idx >= 0 and cur_idx < _video_prev_frame_idx:
        _init_finger_boost_joints()

        for side in ["right", "left"]:
            joint_ids = (
                _finger_boost_state["right_ids"]
                if side == "right"
                else _finger_boost_state["left_ids"]
            )
            ...
            robot.write_joint_state_to_sim(
                q_default,
                torch.zeros_like(q_default),
                joint_ids=joint_ids,
            )
            robot.set_joint_position_target(q_default, joint_ids=joint_ids)

        print("[FINGER RESET] video loop detected; reset fingers to default/open pose")

    _video_prev_frame_idx = cur_idx



def _apply_finger_curl_boost():
    # Current video is mostly right hand, but keep both available.
    _apply_finger_curl_boost_for_side("right")
    _apply_finger_curl_boost_for_side("left")
...
while simulation_app.is_running():
    ...
    _reset_fingers_to_default_on_video_loop()
    _apply_finger_curl_boost()
    scene.write_data_to_sim()
    sim.step()
